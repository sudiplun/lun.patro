...
// Exact BS month lengths, vendored from medic/bikram-sambat's Apache-2.0
// test-data/daysInMonth.json (https://github.com/medic/bikram-sambat).
// These are calendar data, not a fixed AD offset: month lengths vary by year.
// The supported range deliberately brackets present-day use (2013–2034 AD).
var BS_BASE_YEAR = 2070
var BS_BASE_AD = Date.UTC(2013, 3, 14) // 1 Baishakh 2079 = 14 April 2022
...
var NEPALI_MONTHS = ["Baishakh","Jestha","Ashadh","Shrawan","Bhadra",
  "Ashwin","Kartik","Mangsir","Poush","Magh","Falgun","Chaitra"]
var NEPALI_MONTHS_NP = ["बैशाख","जेठ","असार","साउन","भदौ","असोज",
...
// AD Date -> { year, month, day } in BS. month is 0-indexed (0 = Baishakh).
function bsDateFor(adDate) {
  var utcMs = Date.UTC(adDate.getFullYear(), adDate.getMonth(), adDate.getDate())
  var remaining = Math.round((utcMs - BS_BASE_AD) / MS_PER_DAY)
  var year = BS_BASE_YEAR
  while (remaining < 0) {
    year--
    if (!BS_MONTH_LENGTHS[year]) return null
    remaining += bsYearLength(year)
  }
  while (remaining >= bsYearLength(year)) {
    remaining -= bsYearLength(year)
    year++
    if (!BS_MONTH_LENGTHS[year]) return null
  }
  var months = BS_MONTH_LENGTHS[year]
  var mi = 0
  while (remaining >= months[mi]) { remaining -= months[mi]; mi++ }
  return { year: year, month: mi, day: remaining + 1 }
}
...
function bsYearLength(year) {
  ...
  if (!months) return 0
  var total = 0
  for (var i = 0; i < months.length; i++) total += months[i]
  return total
}

function bsDaysInMonth(year, month) {
  var months = BS_MONTH_LENGTHS[year]
  return months && month >= 0 && month < 12 ? months[month] : 0
}

function bsToAd(year, month, day) {
  if (!bsDaysInMonth(year, month) || day < 1 || day > bsDaysInMonth(year, month)) return null
  var offset = 0
  var y
  if (year >= BS_BASE_YEAR) {
    for (y = BS_BASE_YEAR; y < year; y++) { if (!BS_MONTH_LENGTHS[y]) return null; offset += bsYearLength(y) }
  } else {
    for (y = BS_BASE_YEAR - 1; y >= year; y--) { if (!BS_MONTH_LENGTHS[y]) return null; offset -= bsYearLength(y) }
  }
  for (var m = 0; m < month; m++) offset += bsDaysInMonth(year, m)
  return new Date(BS_BASE_AD + (offset + day - 1) * MS_PER_DAY)
}

function bsKey(year, month, day) { return year + "-" + pad2(month + 1) + "-" + pad2(day) }

function bsAddDays(year, month, day, delta) {
  var ad = bsToAd(year, month, day)
  if (!ad) return null
  return bsDateFor(new Date(ad.getTime() + Number(delta) * MS_PER_DAY))
}

function stepBsMonth(year, month, delta) {
  var target = Number(month) + Number(delta)
  var nextYear = Number(year) + Math.floor(target / 12)
  var nextMonth = ((target % 12) + 12) % 12
  return BS_MONTH_LENGTHS[nextYear] ? { year: nextYear, month: nextMonth } : null
}

function bsMonthGrid(year, month, todayKey) {
  var first = bsToAd(year, month, 1)
  if (!first) return []
  var cursor = bsAddDays(year, month, 1, -first.getDay()) // BS UI is Sunday-first.
  var weeks = []
  for (var w = 0; w < 6; w++) {
    var days = []
    var weekAnchor = null
...
// One-line Nepali label, e.g. "१५ असार २०८२".
function nepaliDateLabel(adDate) {
  var bs = bsDateFor(adDate)
  if (!bs) return ""
  return toDevanagari(bs.day) + " " + NEPALI_MONTHS_NP[bs.month] + " " + toDevanagari(bs.year)
}
...
if (typeof module !== "undefined") {
  module.exports = {
    ...
    nepaliDateLabel: nepaliDateLabel,
